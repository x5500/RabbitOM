...
namespace RabbitOM.Net.Rtp.H265
{
    ...
    {
        private static readonly int DefaultMinimumLength = 5;
        private H265NalUnit() { }
        ...
        public static bool TryParse(byte[] buffer, out H265NalUnit result)
        {
            ...
            result = new H265NalUnit
            {
                ...
                Type = (byte)((buffer[index] >> 1) & 0x3F),
                LayerId = (byte)(((buffer[index] << 7) & 0x80) | ((buffer[index] >> 3) & 0x1F)),
                TID = (byte)((buffer[index]) & 0x03),
                Payload = new byte[buffer.Length - ++index],
            };
